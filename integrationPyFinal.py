...
OBJECT_QUERIES = {
    "OContactRecording__c": """
        ...
        WHERE Agreement__r.Status__c = 'Closed'
          AND Agreement__r.NPASTAGE__c = 'REGULAR' AND Migrated_to_Mongo__c = FALSE
    """,
    ...
    "ODMS_File__c": """
        ...
        sObject_Name__c,sObject_Record_Id__c,Vertical__c,OwnerId,Owner.Name 
...
def log_error(sobject_name, record_id, stage, message, record_data=None):
    ...
    try:
        ...
        print(f" Logged error for {sobject_name} ({record_id}) at {stage}")
...
def sync_salesforce_object(sobject_name, base_query):
    ...
    base_where = "WHERE" if "WHERE" not in base_query.upper() else "AND"

    if last_sync:
        soql = (
            f"{base_query} {base_where} Migrated_to_Mongo__c = FALSE "
            f"AND SystemModstamp > {last_sync.isoformat()} "
            f"ORDER BY SystemModstamp ASC LIMIT 50000"
        )
    else:
        soql = (
            f"{base_query} {base_where} Migrated_to_Mongo__c = FALSE "
            f"ORDER BY CreatedDate ASC LIMIT 50000"
        )
    ...
    for rec in records:
        ...
        vertical = (rec.get("Vertical__c") or "Unknown").strip().upper()
        if vertical not in ["SME", "LAP", "HL"]:
            vertical = "Other"
        coll_name = f"{sobject_name}_{vertical}".replace(".", "_")
...
# ------------------------------------------------------
#  Mark records as Migrated in Salesforce (Correct Spot)
# ------------------------------------------------------
            ...
            sf.restful("composite/sobjects", method="PATCH", data=update_payload)

            print(f" Marked {len(record_ids)} records as Migrated in Salesforce")

# -------------------------------------------------------------
# Delete previous error logs for successfully upserted records
# -------------------------------------------------------------
            deleted_result = error_coll.delete_many({
                "sobject_name": sobject_name,
                "record_id": {"$in": record_ids}
            })
            if deleted_result.deleted_count:
                print(f" Deleted {deleted_result.deleted_count} previous error logs for {coll_name}")
